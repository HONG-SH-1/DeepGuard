...
| 4 | 2차 모델(Bi-LSTM) 학습 지연/저하 | 1차 모델의 `relu`를 2차에도 동일 적용하여 기울기 문제 발생 | 긴 시퀀스(W=20)에 적합한 `tanh`로 변경 후 속도 38% 향상 |
| 5 | 대시보드 상태 초기화 버그 | Streamlit 버튼 클릭 시마다 전체 스크립트가 재실행됨 | `st.session_state`를 적용하여 데이터 전처리 및 분석 상태 유지 |
